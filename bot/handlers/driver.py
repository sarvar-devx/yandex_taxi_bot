...
from bot.states.user import DriverStates
...
@driver_router.message(DriverStates.car_brand)
async def change_car_brand_handler(message: Message, state: FSMContext):
    ...
    await Driver.update(message.from_user.id, car_brand=message.text.title())
